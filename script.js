...
// 🚀 Magic Link Generator Engine + AUTOMATIC URL SHORTENER ⚡
document.getElementById('generate-btn').addEventListener('click', async () => {
    ...
    genBtn.innerText = "Generating Short Link... ⏳";
    ...
    try {
        ...
        alert("Kuch dikkat aayi ya data zyada bada ho gaya! Ek baar kam photos ke sath try karein.");
        console.error(error);
    } finally {
...
document.getElementById('copy-btn').addEventListener('click', () => {
    ...
    alert("Interactive Short Link copied! Share via WhatsApp now 📲");
});

// 🔍 SMART DECODER ENGINE
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        const cleanBase64 = surpriseParam.trim().split(' ')[0];
        ...
        console.log("Invalid parameter bundle.");
        document.getElementById('setup-form').classList.remove('hidden');
    }
});
